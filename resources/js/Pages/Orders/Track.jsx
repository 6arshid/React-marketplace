...
const TrackingIcon = ({ className = "w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8" }) => (
...
const ShippingIcon = ({ className = "w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" }) => (
    <svg className={className} fill="currentColor" viewBox="0 0 24 24">
...
const DownloadIcon = ({ className = "w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" }) => (
    <svg className={className} fill="currentColor" viewBox="0 0 24 24">
        <path d="M5,20H19V18H5M19,9H15V3H9V9H5L12,16L19,9Z"/>
    </svg>
);

const FileIcon = ({ className = "w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" }) => (
    <svg className={className} fill="currentColor" viewBox="0 0 24 24">
    ...
    switch (status.toLowerCase()) {
        ...
        case 'processing':
            return {
                ...
                icon: <ShippingIcon className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500" />,
            ...
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-6 sm:py-12">
                <div className="mx-auto max-w-4xl px-3 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 sm:mb-8 text-center">
                        <div className="mx-auto mb-3 sm:mb-4 flex h-12 w-12 sm:h-14 sm:w-14 lg:h-16 lg:w-16 items-center justify-center rounded-full bg-gradient-to-r from-blue-500 to-indigo-600">
                            <TrackingIcon className="text-white" />
                        </div>
                        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{t('Order Tracking')}</h1>
                        <p className="mt-2 text-sm sm:text-base text-gray-600 px-4">{t('Track your order status and download files')}</p>
                    </div>

                    {/* Tracking Code Card - Mobile Responsive */}
                    <div className="mb-6 sm:mb-8 overflow-hidden rounded-xl sm:rounded-2xl bg-white shadow-lg sm:shadow-xl ring-1 ring-gray-200">
                        <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-4 sm:px-6 py-3 sm:py-4">
                            <h2 className="text-base sm:text-lg font-semibold text-gray-900">{t('Order Information')}</h2>
                        </div>
                        <div className="p-4 sm:p-6">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                                <div>
                                    <p className="text-xs sm:text-sm font-medium text-gray-600">{t('Tracking Code')}</p>
                                    <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 font-mono">#{order.tracking_code}</p>
                                </div>
                                <div className="sm:text-right">
                                    <p className="text-xs sm:text-sm font-medium text-gray-600">{t('Order Status')}</p>
                                    <div className="mt-1 flex items-center sm:justify-end space-x-2">
                                        {statusDetails.icon}
                                        <span className={`inline-flex items-center px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium border ${statusDetails.bgColor} ${statusDetails.color} ${statusDetails.borderColor}`}>
                    ...
                    <div className={`mb-6 sm:mb-8 rounded-xl sm:rounded-2xl border p-4 sm:p-6 ${statusDetails.bgColor} ${statusDetails.borderColor}`}>
                        <div className="flex items-center space-x-3">
                            {statusDetails.icon}
                            <div>
                                <h3 className={`font-semibold text-sm sm:text-base ${statusDetails.color}`}>
                                    {order.status.charAt(0).toUpperCase() + order.status.slice(1)}
                                </h3>
                                <p className={`text-xs sm:text-sm ${statusDetails.color}`}>
                    ...
                    {/* Postal Tracking - Mobile Responsive */}
                    {order.postal_tracking_code && (
                        <div className="mb-6 sm:mb-8 overflow-hidden rounded-xl sm:rounded-2xl bg-white shadow-lg sm:shadow-xl ring-1 ring-gray-200">
                            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-4 sm:px-6 py-3 sm:py-4">
                                <div className="flex items-center space-x-3">
                                    <div className="rounded-lg bg-blue-100 p-1.5 sm:p-2">
                                        <ShippingIcon className="text-blue-600" />
                                    </div>
                                    <h3 className="text-base sm:text-lg font-semibold text-gray-900">{t('Shipping Information')}</h3>
                                </div>
                            </div>
                            <div className="p-4 sm:p-6">
                                <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                                    <p className="text-xs sm:text-sm font-medium text-gray-600">{t('Postal Tracking Code:')}</p>
                                    <span className="inline-flex items-center px-2 sm:px-3 py-1 rounded-lg bg-blue-50 text-blue-800 font-mono text-xs sm:text-sm border border-blue-200 break-all">
                                        {order.postal_tracking_code}
                                    </span>
                                </div>
                    ...
                    {/* Downloads Section - Mobile Responsive */}
                    {files.length > 0 && canDownload && (
                        <div className="overflow-hidden rounded-xl sm:rounded-2xl bg-white shadow-lg sm:shadow-xl ring-1 ring-gray-200">
                            <div className="bg-gradient-to-r from-green-50 to-emerald-50 px-4 sm:px-6 py-3 sm:py-4">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <h3 className="text-base sm:text-lg font-semibold text-gray-900">{t('Available Downloads')}</h3>
                                            <p className="text-xs sm:text-sm text-gray-600">{files.length} {files.length !== 1 ? t('files') : t('file')} {t('ready for download')}</p>
                                        </div>
                                    </div>
                            ...
                            <div className="p-4 sm:p-6">
                                    ...
                                    {files.map((file, idx) => (
                                        <div key={idx} className="group relative">
                                            ...
                                            >
                                                <div className="flex items-start space-x-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-start justify-between">
                                                            <p className="text-sm font-medium text-gray-900 truncate group-hover:text-green-600 transition-colors pr-2">
                                                                {file.title}
                                                            </p>
                                                            <ExternalLinkIcon className="flex-shrink-0 text-gray-400 group-hover:text-green-500 transition-colors" />
                                                        </div>
                                                ...
                                                <div className="absolute inset-0 bg-green-500 bg-opacity-0 group-hover:bg-opacity-5 rounded-lg sm:rounded-xl transition-all duration-200 hidden sm:flex items-center justify-center opacity-0 group-hover:opacity-100">
                                ...
                                {/* Download All Button - Mobile Responsive */}
                                {files.length > 1 && (
                                    <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-200">
                                        <button className="inline-flex items-center w-full sm:w-auto justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors duration-200 shadow-sm hover:shadow-md text-sm sm:text-base">
